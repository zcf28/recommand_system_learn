...
class ChurnPredWithGBDTAndLR:
    """
    功能描述： GBDT+LR模型 电信客户流失预测
    """

    def __init__(self):
        self.file = "./processed_data/new_churn.csv"
    ...
    # 拆分数据集
    def split(self):
        train, test = train_test_split(self.data, test_size=0.3, random_state=40)
        return train, test
    ...
    def train_model(self):
        ...
        # 创建gbdt模型 并训练
        gbdt = GradientBoostingClassifier(learning_rate=0.1, n_estimators=500, max_depth=7)
        ...
        print(gbdt.apply(x_train).reshape(-1, 500).shape)

        # 500为n_estimators，迭代次数
        enc.fit(gbdt.apply(x_train).reshape(-1, 500))
        gbdt_lr.fit(enc.transform(gbdt.apply(x_train).reshape(-1, 500)), y_train)
    ...
    def evaluate(self, enc, gbdt, lr, gbdt_lr):
        ...
        gbdt_lr_y_pred = gbdt_lr.predict_proba(enc.transform(gbdt.apply(x_test).reshape(-1, 500)))
